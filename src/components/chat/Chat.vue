    ...
    <div class="container">
      ...
      <div class="dialog-box scrollable-div">
        <div v-for="dialog in dialogList" :key="dialog.index" :class="['dialog-bubble', { 'user-content': !dialog.isResponse }]">
          <svg v-if="dialog.content === ''"
          t="1703209866887" class="loading-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5281" width="200" height="200"><path d="M480 96a32 32 0 0 1 64 0v192a32 32 0 0 1-64 0V96z m250.624 60.64a32 32 0 1 1 51.776 37.632L669.568 349.6a32 32 0 0 1-51.808-37.632L730.624 156.64zM897.76 353.024a32 32 0 1 1 19.776 60.864l-182.624 59.328a32 32 0 0 1-19.776-60.864l182.624-59.328z m19.776 257.088a32 32 0 1 1-19.776 60.864l-182.624-59.328a32 32 0 0 1 19.776-60.864l182.624 59.328zM782.4 829.76a32 32 0 0 1-51.776 37.632L617.76 712.064a32 32 0 1 1 51.808-37.632L782.4 829.76zM544 928a32 32 0 0 1-64 0V736a32 32 0 0 1 64 0v192z m-250.624-60.64a32 32 0 0 1-51.776-37.632L354.432 674.4a32 32 0 0 1 51.808 37.632L293.376 867.36zM126.24 670.976a32 32 0 1 1-19.776-60.864l182.624-59.328a32 32 0 0 1 19.776 60.864L126.24 670.976z m-19.776-257.088a32 32 0 0 1 19.776-60.864l182.624 59.328a32 32 0 1 1-19.776 60.864l-182.624-59.328zM241.6 194.24a32 32 0 1 1 51.776-37.632L406.24 311.936a32 32 0 1 1-51.808 37.632L241.6 194.24z" p-id="5282"></path></svg>
      ...
      submit() {
        ...
        Chat.createCompletion(data).then(
            async (response) => {
              ...
              //     console.log(chunk)
                  // const processedChunk = `[${chunk.toString().replace(/}{/g, '},{')}]`;
                  // const responseItems = JSON.parse(processedChunk);

                  // responseItems.forEach((item, index) => {
                  //   // 处理响应项
                  //   if (index !== responseItems.length - 1) {
                  //     // 处理消息内容
                  //     const content = item.delta.content;
                  //     // ...

                  //     // 更新对话列表
                  //     this.dialogList[this.dialogList.length - 1].content += content;
                  //   } else {
                  //     // 处理会话ID
                  //     this.conversationID = item.delta.conversation.id;
                  //     // ...
                  //   }
                  // });
